...
class RenderDetails extends Component {
  renderTimeStamp() {
    if (this.props.event.startTime !== undefined) {
      return (
        <Timestamp value={this.props.event.startTime} />
      )
    }
    else {
      return (
    ...
    return (
      <div style={{ padding: '20px 40px 0 0' }}>
        ...
        <div id="share-buttons">
          ...
          <Button color="facebook" href={`http://www.facebook.com/sharer.php?u=${window.location.href}`}>
            <i
              className={'fab fa-facebook-square'}
            />{' '}
            Share
        </Button>
          <Button href={`https://twitter.com/share?url=${window.location.href}`} color="twitter">
            <i
              className={'fab fa-twitter'}
            />{' '}
            Share with Twitter
        </Button>
          <Button color="google" href={`https://plus.google.com/share?url=${window.location.href}`}>
            <i
