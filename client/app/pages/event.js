...
class Event extends Component {
  ...
  renderMap() {
    const { event } = this.props;
      ...
      const MyMapComponent = withScriptjs(withGoogleMap((props) =>
        ...
        >
          {props.isMarkerShown && <Marker position={{ lat: event.loc[0], lng: event.loc[1] }} />}
        </GoogleMap>
      ));
      return (
        <MyMapComponent
  ...
  render() {
    ...
    if (loading) {
      return <div align='center'><Spinning size={'large'} /> loading Map...</div>;
    }
...
export default compose(withStyles(landingPageStyle, pillsStyle), connect(mapStateToProps, { fetchSpecificEvent }))(Event);